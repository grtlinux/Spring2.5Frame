...
public class SubmitReportControllerTest extends TestCase {
	...
	public void testValidReportSubmit() throws IOException {
		...
		multipartRequest.addFile(new MockMultipartFile("reportFile",
				"temp/SpringReport.doc", null, getBytesFromFile()));
		multipartRequest.addFile(new MockMultipartFile("sourceCode",
				"temp/SubmitReportControllerTest.java", null,
				getBytesFromFile()));
		...
		try {
			...
			assertEquals("temp/SpringReport.doc", command.getReportFile().getOriginalFilename());
	...
	public void testInvalidReportSubmit() throws IOException {
		...
		request.addFile(new MockMultipartFile("reportFile",
				"temp/SpringReport.doc", null, getBytesFromFile()));
		try {
			ModelAndView mav = reportController
					.handleRequest(request, response);
